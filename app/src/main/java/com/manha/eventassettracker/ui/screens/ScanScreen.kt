...
fun ScanScreen(viewModel: AppViewModel, onBack: () -> Unit) {
    ...
    ScreenScaffold(
        title = if (scanMode == ScanType.OUT) "OUT Scan" else "RETURN Scan",
        onBack = onBack
    ) { padding ->
        Column(
            modifier = Modifier
            ...
            EditableEventField(
                ...
                onCreateNew = { name ->
                    viewModel.addEvent(name, "", "", "") { event ->
                        viewModel.setActiveEvent(event)
                    }
                }
            )
            ...
            Row(
                ...
                Button(
                    ...
                    colors = if (scanMode == ScanType.OUT)
...
private fun EditableEventField(
    ...
    Column {
        ...
        if (text.isNotBlank() && !matchesExisting) {
            Spacer(Modifier.height(6.dp))
            Button(
                onClick = { onCreateNew(text.trim()) },
                modifier = Modifier.fillMaxWidth()
            ) {
